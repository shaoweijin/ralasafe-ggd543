...
public class LoginFilter implements Filter {
    ...
    private boolean requestFromLoginPage(HttpServletRequest httpReq) {
        String refererUrl = httpReq.getHeader("REFERER");
        String url = StringUtil.isEmpty(refererUrl) ? httpReq.getHeader("referer") : refererUrl;
    ...
    private void gotoLoginPage(HttpServletRequest httpReq,
        ...
        HttpSession session = httpReq.getSession();
        session.setAttribute(RALASAFE_REQUEST_LOGIN, "y");
        session.setAttribute(DENY_MESSAGE, denyMessage);
        session.setAttribute(GOTO_PAGE, gotoPage);
    ...
    public void init(FilterConfig config) throws ServletException {
        String rawUniqueFieldsParams = config.getInitParameter("uniqueFieldsParams");
        if (!StringUtil.isEmpty(rawUniqueFieldsParams)) {
            uniqueFieldsParams = StringUtil.splitAndTrim(rawUniqueFieldsParams, ",");
        } else {
            uniqueFieldsParams = new String[0];
        }

        String _passwordParam = config.getInitParameter("passwordParam");
        passwordParam = StringUtil.isEmpty(_passwordParam) ? "password" : _passwordParam;

        String _userPasswordField = config.getInitParameter("userPasswordField");
        userPasswordField = StringUtil.isEmpty(_userPasswordField) ? "password" : _userPasswordField;

        String _denyMessage = config.getInitParameter("denyMessage");
        denyMessage = StringUtil.isEmpty(_denyMessage) ? "LoginName and password not matched" : _denyMessage;

        initEncrypt(config);
        String _loginPage = config.getInitParameter("loginPage");
        loginPage = StringUtil.isEmpty(_loginPage) ? "/ralasafe/demo/login.jsp" : _loginPage;
    ...
    private void initEncrypt(FilterConfig config) throws ServletException {
